...
contract ImpermanentGain is ERC20Mintable {
    ...
    bytes32 public constant SIG_TYPEHASH = keccak256("MintWithSig(address maker,uint256 amount,uint256 price,bool isA,uint256 nonce,uint256 expiry)");
    ...
    function mintWithSig(address maker, uint256 amount, uint256 price, bool isA,
        uint256 nonce, uint256 expiry, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        bytes32 digest =
            keccak256(abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(SIG_TYPEHASH,
                                     maker,
                                     amount,
                                     price,
                                     isA,
        ...
        require(maker == ecrecover(digest, v, r, s), "invalid-Sig");
        require(expiry == 0 || now <= expiry, "order-expired");
        require(!filledTx[digest], "order-filled");

        filledTx[digest] = true;

        if(isA) {
            a[maker] = a[maker].add(amount);
            b[msg.sender] = b[msg.sender].add(amount);
        }
        else {
            a[msg.sender] = a[msg.sender].add(amount);
            b[maker] = b[maker].add(amount);
        }

        uint256 makerPayment = amount.mul(price).div(1e18);
        uint256 takerPayment = amount.sub(makerPayment);
        doTransferIn(baseToken, maker, makerPayment);
        doTransferIn(baseToken, msg.sender, takerPayment);
    }
